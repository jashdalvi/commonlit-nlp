...
initialize(version_base=None, config_path="config", job_name = "train")
...
subprocess.run(["kaggle", "datasets", "init", "-p", cfg.output_dir])
kaggle_dataset_metadata = {
  "title": f"commonlit-kaggle-{cfg.model_name.split(os.path.sep)[-1]}-cv-{cv:.4f}",
  "id": f"jashdalvi99/commonlit-kaggle-{cfg.model_name.split(os.path.sep)[-1]}-cv-{cv:.4f}",
  "licenses": [
    {
      "name": "CC0-1.0"
    }
...
subprocess.run(["kaggle", "datasets", "create", "-p", cfg.output_dir])
