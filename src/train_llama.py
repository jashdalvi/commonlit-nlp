...
def main(cfg: DictConfig):
    ...
    class Model(nn.Module):
        ...
        def forward(self, ids, mask, targets = None):
            transformer_out = self.transformer(input_ids = ids, attention_mask = mask, output_hidden_states = True)
            logits = self.pool(transformer_out.hidden_states[-1], mask, transformer_out.hidden_states)
            return logits
    ...
    def get_optimizer_scheduler(model, num_train_steps):
        ...
        no_decay = [nd.lower() for nd in ["bias", "LayerNorm.weight"]]
        optimizer_params = [
            {
